...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = (from e in emps let sum = e.Sal + (e.Comm ?? 0) select new { e.EName, Total = sum }).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d }).Join(grades, ed => ed.e.Sal, s => s.Losal, (ed, s) => new { ed.e.EName, ed.d.DName, s.Grade }).Where(r => emps.Any(e => e.Sal >= grades.Min(g => g.Losal) && e.Sal <= grades.Max(g => g.Hisal))).ToList();
